...
void LEM_CWEA::Timestep(double simdt) {
	...
	if (IsCWEAPowered())
	{
		...
		LRHeaterCautFF.Set((lem->pMission->GetLMNumber() < 4 && (lem->scera1.GetVoltage(20, 4) < 2.30 || lem->scera1.GetVoltage(20, 4) > 4.36)) ||
		...
		QD1HeaterCautFF.Set((lem->pMission->GetLMNumber() < 7 && (lem->scera1.GetVoltage(20, 4) < 2.79 || lem->scera1.GetVoltage(20, 4) > 4.725)) ||
			(lem->pMission->GetLMNumber() > 6 && (cappedvoltage < 2.79 || cappedvoltage > 4.725)));
